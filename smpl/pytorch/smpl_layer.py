...
class SMPL_Layer(Module):
    ...
    def forward(self,
                th_pose_coeffs,  # [1,72]
                th_betas=torch.zeros(1),  #[1,10]
                th_trans=torch.zeros(1)): #[0,]
        """
        Args:
        ...
        th_rot_map = th_pose_coeffs[1:24].reshape((1, 207)).float()

        th_pose_map = subtract_flat_id(th_rot_map).float()

        root_rot = th_pose_coeffs[0].reshape((1,3,3)).float()
        ...
        if th_betas is None or bool(torch.norm(th_betas) == 0):
            ...
            t1 = th_betas.transpose(1, 0)
            t2 = self.th_shapedirs
            t3 = torch.matmul(self.th_shapedirs, th_betas.transpose(1, 0)).permute(2, 0, 1)
                            #  [6890, 3, 10]       [10,1]                       [1, 6890, 3]
            ...
            # [1,24, 3]        # [24, 6890]          # [1, 6890, 3]
            t4 = th_pose_map.transpose(0, 1)
            t5 = torch.matmul(self.th_posedirs, th_pose_map.transpose(0, 1))
        th_v_posed = th_v_shaped + torch.matmul(self.th_posedirs, th_pose_map.transpose(0, 1)).permute(2, 0, 1)
                   # [1, 6890, 3]                # [6890, 3, 207]         [207,1]
        ...
        #[1, 3, 1]
        t6 = torch.cat([root_rot, root_j], 2)
        t7 = th_with_zeros(t6)
        th_results.append(th_with_zeros(torch.cat([root_rot, root_j], 2)))  # 旋转加位移  1， 4， 4
                                                  #[1,3,3]   [1,3,1]
        ...
        # [1,4, 6890]
        th_verts = (th_T * th_rest_shape_h.unsqueeze(1)).sum(2).transpose(2, 1)  #################  th_verts
        # [1, 6890, 4]     # 在第一个维度增加一个维度[1,1,4,6890]
        th_verts = th_verts[:, :, :3]                                            #################  th_verts
        # [1, 6890, 3]
        th_jtr = torch.stack(th_results_global, dim=1)[:, :, :3, 3]               #################  th_jtr
        # [1, 24, 3]
        if th_trans is None or bool(torch.norm(th_trans) == 0):
            if self.center_idx is not None:
                center_joint = th_jtr[:, self.center_idx].unsqueeze(1)             #################  th_jtr
                th_jtr = th_jtr - center_joint                                    #################  th_jtr
                th_verts = th_verts - center_joint                               #################  th_verts
        else:
            th_jtr = th_jtr + th_trans.unsqueeze(1)                               #################  th_jtr
            th_verts = th_verts + th_trans.unsqueeze(1)                           #################  th_verts
