...
def display_model(
        model_info,        # {'verts': verts.cpu().detach(),   'joints': Jtr.cpu().detach()}
        model_faces=None,  # model_faces=smpl_layer.th_faces,
        with_joints=False, # with_joints=True,
        kintree_table=None,# kintree_table=smpl_layer.kintree_table,
        ax=None,
        batch_idx=0,
        show=True,
        savepath=None):    # savepath='image.png',
    """
    ...
    if model_faces is None:
        ...
        ax.add_collection3d(mesh)  # 画的是 网格 每个点vetex的坐标
    if with_joints:
...
def draw_skeleton(joints3D, kintree_table, ax=None):
    ...
    # For each 24 joint
    for i in range(1, kintree_table.shape[1]):      # 画的是 棍图
