...
def th_posemap_axisang(pose_vectors):  # [1,72]  pose_params = torch.rand(batch_size, 72) * 0.2

    _pose_vectors = pose_vectors # [1,72]
    rot_nb = int(pose_vectors.shape[1] / 3) # 72/3 = 24
    rot_mats = []
    for joint_idx in range(rot_nb - 1):
        joint_idx_val = joint_idx + 1
        axis_ang = pose_vectors[:, joint_idx_val * 3:(joint_idx_val + 1) * 3] # [1,3]

        rot_mat = rodrigues_layer.batch_rodrigues(axis_ang)  # [1,9]

        rot_mats.append(rot_mat)

    # rot_mats = torch.stack(rot_mats, 1).view(-1, 15 *9)
    rot_mats = torch.cat(rot_mats, 1)  # [1,207]
    pose_maps = subtract_flat_id(rot_mats)  # 减掉 单位矩阵
